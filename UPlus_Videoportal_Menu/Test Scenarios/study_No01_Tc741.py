...
@stepResult(descriptionMsg=u"교과 > 홈 버튼, 타이틀, 검색, GNB 버튼")       
def TC741():
    ...
    result = False
    for i in range(0,20):
        rc = DEV1.ScrollDown("avwListView", 9002)
        if rc != AT_SUCCESS:
            System.Finish(ExecutionResult.T_FAILED, rc)
            
        searchingText = u"초중고 교과"
    ...
    #찾으면 클릭
    rc = DEV1.UIClickByText(u"초중고 교과")
    if rc != AT_SUCCESS:
        return rc
    ...
    #클릭 후 실시간채널 있는지 확인    
    rc, result = DEV1.UIWaitForText(u'실시간채널', 500)
    ...
    #초중고 교과 진입 했는지 확인
    rc, result = DEV1.UIWaitForText(u'초중고 교과', 500)
    if rc != AT_SUCCESS:  # API가 성공적으로 수행되었는지 확인
        return rc
